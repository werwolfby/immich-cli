...
async function upload(
  paths: string[],
  { key, server, recursive, yes: assumeYes, delete: deleteAssets, uploadThreads, album: createAlbums, skipHash }: any,
) {
...
async function startUpload(endpoint: string, key: string, asset: any) {
  try {
    ...
    log();
    log(`${asset.path}: ${e.reason?.response?.status ?? e.reason ?? e} ${e.reason?.response?.statusText ?? ''}`);
    errorAssets.push({
      file: asset.path,
      reason: e,
...
async function pingServer(endpoint: string) {
  try {
    ...
    log(chalk.red('Error connecting to server - check server address and port'), e.message);
    process.exit(1);
  }
}
